...
import { z } from "zod";
import { cid, id, consumerCommands, schemas } from "./contracts.mjs";
...
const commands = Object.fromEntries(Object.entries(consumerCommands).map(([command, operation]) => [command.replace("consumer.", "").replaceAll(".", "_"), { command, operation }]));
class AgentHarness {
