...
const consumers=new Set(Object.keys(consumerCommands).map(consumerToolName));
...
export function createProductTools(session,{timeoutMs=30000,pollMs=20,profile='personal',onRetainedMessage=()=>{}}={}) {
 const retained=new Map();const descriptors=new Map();const notified=new WeakSet();
 ...
 async function execute(name,input={}) {
  ...
  try {
   ...
   await ctx.check();const output=name==='ac_messages'&&Array.isArray(value?.messages)?{...value,messages:value.messages.map(item=>applicationMessage(item,ctx.row.contact_cid))}:value;const result=bounded(output);
   // Polling commands also consume ordinary chat into the retained inbox. Wake
   // the host after identity validation, even when daemon unread is now empty.
   if(name!=='ac_messages')for(const item of ctx.harness.messages){
    if(item&&typeof item==='object'&&!notified.has(item)){onRetainedMessage(ctx.row,item);notified.add(item);}
   }
