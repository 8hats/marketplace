...
export class ConnectionRegistry {
 ...
 async read(file){const handle=await fs.open(file,fs.constants.O_RDONLY|fs.constants.O_NOFOLLOW);try{const stat=await handle.stat();if(!stat.isFile()||(!WIN&&(stat.uid!==process.getuid()||(stat.mode&0o077))))throw error('connection_registry_unsafe');return JSON.parse(await handle.readFile('utf8'));}finally{await handle.close();}}
 async write(file,row,exclusive=false){const target=exclusive?file:file+'.'+randomUUID()+'.tmp';const handle=await fs.open(target,'wx',0o600);try{await handle.writeFile(JSON.stringify(row)+'\n');await handle.sync();}finally{await handle.close();}if(!exclusive)await fs.rename(target,file);const dir=await fs.open(this.root,'r');try{if(!WIN)await dir.sync();}finally{await dir.close();}}
