...
import { cid, id, limits, consumerCommands, consumerToolName, schemas } from "./contracts.mjs";
...
function registerAgentTools(register, client, configuration) {
  ...
  for (const [command, operation] of Object.entries(consumerCommands)) {
    const name = consumerToolName(command);
    add(name, command === "consumer.ac.artifact.result.submit" ? "Explicitly submit a result summary and 1–20 existing attachment IDs with their exact displayed SHA-256 hashes for subsequent human Owner approval. Read attachment metadata first; never infer approval or automatically submit from positive review feedback. Supply a caller-generated stable idempotency_key. On unknown outcome inspect artifact_result and ac_messages; never automatically retry or invent a new key." : command === "consumer.ac.file.review.request" ? "Request an identified human to review verified attachment bytes. Supply a caller-generated stable idempotency_key. On unknown outcome inspect file_review; never automatically retry or invent a new key." : `Call ${command} under current product authorization.`, schemas[operation], (input) => harness.call(name, input));
  }
