...
export class CentralClient {
 constructor({fetchFn=fetch,store=new ConnectionStore(),origin=process.env.AC_COWORK_URL,onEvent=()=>{}}={}){
  const incompatible=['OURS_PORT','OURS_STATE_DIR','OURS_CONFIG','OURS_URL','OURS_TOKEN','OURS_TOKEN_FILE','AU_OURS_URL','AU_OURS_API_TOKEN','AU_OURS_CONFIG','AC_REMOTE_OURS_URL','AC_REMOTE_OURS_TOKEN_FILE'].some(key=>process.env[key]);
