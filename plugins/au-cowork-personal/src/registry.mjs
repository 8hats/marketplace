...
export class RoomRegistry {
  constructor(stateDir, { appHome = process.env.AC_LEGACY_ROOM_REGISTRY ?? path.join(stateDir, 'cowork-personal-legacy') } = {}) {
    if(!path.isAbsolute(appHome)||!path.isAbsolute(stateDir))throw new Error('Absolute daemon and legacy registry paths are required.');
