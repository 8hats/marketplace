...
test('plugin advertises 16 strict Personal Agent ac tools and correlates one actual room command',async()=>{
 const f=fixture();const runtime=await createRuntime(f);try{
 assert.equal(f.tools.size,22);assert.equal(f.tools.has('ac_request_decide'),true);
...
test('chat consumed while polling a command wakes the host and remains readable',async()=>{
 const envelope={version:1,kind:'room_msg',room_id:'r1',room_name:'Canonical Room',at:'2026-09-16T07:08:57.380Z',author:{display_name:'Peer',identity:'C'.repeat(64),role:'Personal Agent'},message_id:'chat',text:'Private retained chat'};
 const chat={wire_id:'chat-wire',direction:'in',from:{id:room},body:JSON.stringify(envelope)};
 let reads=0;const f=fixture({getMessages:async()=>{reads++;return {messages:reads===1?[chat,{...chat,wire_id:'foreign',from:{id:'D'.repeat(64)}}]:[],command_results:[{wire_id:'reply',direction:'in',message_kind:'command_result',from:{id:room},reply_to:{wire_id:'request'},body:JSON.stringify({ok:true,result:{ok:true,result:{status:'ok',data:{id:'room'}}}})}],remaining:0};}}),wakes=[];
 f.server.sendLoggingMessage=value=>wakes.push(JSON.parse(value.data));
 const runtime=await createRuntime(f);try{
 await f.tools.get('ac_read').fn({kind:'room'});
 assert.equal(wakes.length,1);assert.equal(wakes[0].event,'room_message_available');assert.equal(wakes[0].wire_id,'chat-wire');assert.doesNotMatch(JSON.stringify(wakes),/Private retained chat/);
 const waited=JSON.parse((await f.tools.get('wait_for_room_event').fn({timeout_ms:1000})).content[0].text);assert.equal(waited.status,'event');assert.equal(waited.event.wire_id,'chat-wire');
 const mail=JSON.parse((await f.tools.get('ac_messages').fn({})).content[0].text);assert.equal(mail.messages[0].body,chat.body);assert.equal(reads,1);assert.equal(wakes.length,2);
 }finally{await runtime.shutdown();}
});
test('ac_messages returns authenticated application author while preserving original body and acknowledgement',async()=>{
 ...
 const body=JSON.stringify({version:1,kind:'room_msg',room_id:'r',room_name:'Room',message_id:'message',at:'2026-09-16T00:00:00Z',author:{identity:room,display_name:'Web',role:'Web'},text:JSON.stringify(app)});
 let reads=0;const f=fixture({getMessages:async()=>({messages:++reads===1?[{direction:'in',wire_id:'wire',from:{id:room},body}]:[],command_results:[],remaining:0})});const runtime=await createRuntime(f);
 try{const mail=JSON.parse((await f.tools.get('ac_messages').fn({})).content[0].text);assert.equal(mail.messages[0].application_message.author.id,'human');assert.equal(mail.messages[0].body,body);assert.equal(JSON.parse((await f.tools.get('ac_messages').fn({})).content[0].text).messages.length,0);}finally{await runtime.shutdown();}
});
