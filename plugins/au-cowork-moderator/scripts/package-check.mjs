...
try{
 ...
 assert.equal(client.getServerVersion().name,'au-cowork-moderator');
 for(const name of ['ac_request_route','ac_review_publish','ac_publication_propose','ac_intervention_record','ac_stage_explain','ac_result_create'])assert.ok(tools.some(tool=>tool.name===name),name);
 assert.equal(tools.some(tool=>tool.name==='ac_join'),false);
